...
public class HttpUtils {
	...
	public static String sendGet(String url, HashMap<String, String> params) {
		...
		try {
			...
			// connection.addRequestProperty("Cookie", "cookie1");
			// connection.addRequestProperty("Cookie", "cookie2");
	 ...
	 * @return 所代表远程资源的响应结果
	 */
	public static void sendPost(String url, String json, MyResponseCallBack responseCallBack) {
		L.i("send url: " + url);
		L.i("send json: " + json);
		PrintWriter out = null;
		BufferedReader in = null;
		...
		try {
			...
			// 设置cookie
			responseCallBack.addCookie(conn);
			...
			try {
				...
				L.i("response json: " + result);
			} catch (Exception e) {
				L.e("network Exception InputStream 获取服务端数据出现异常!" + e);
			}
			responseCallBack.onSuccess(200, result);
		} catch (Exception e) {
			L.e("network Exception  发送 POST 请求出现异常！" + e);
			responseCallBack.onFailure(e, "发送 POST 请求出现异常！");
		} finally {
			/** 使用finally块来关闭输出流、输入流 **/
			try {
				if (out != null) {
