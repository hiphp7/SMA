...
//	$(".registerform").Validform();  //就这一行代码！
	...
	var index = form.attr("ajaxpost");
	form.Validform({
		
		btnReset:"#reset-btn",//可选项#reset-btn是该表单下要绑定点击重置表单事件的按钮;
...
//					objtip.show();
		...
		callback:function(resp){
			if(resp.status == 0){
				//看form表单是否有success参数
				var scfn = form.attr('success');
				if(scfn != null){
					eval(scfn+'("'+resp.data+'")');
				}else {
					//jQuery("input:eq(0)").val(resp.id);
		...
		datatype:{//传入自定义datatype类型【方式二】;
			"pwd":function(val,obj,curform,datatype){
				//该方法由佚名网友提供;
				var lv = 0;
			    if(val.match(/[a-z]/g)){
			    	lv++;
			    ...
			    pwdStrength.find('span:lt('+lv+')').addClass('bgStrength');
			    if(lv >= 2){
			    	return true;
			    }
			    return false;
