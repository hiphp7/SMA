...
class Agent extends Admin_Controller {
	function __construct() {
		...
		parent::__construct();
		$this->load->model(array('agent_model','sharepage'));
	...
	function index(){
		...
		$like = array();
		$name  = htmlspecialchars($this->input->get_post("agentname"));
		$string = '';
		if(!empty($name)){
			$like['agentName'] = $name;
			$string .="&agentName=".$name;
		}
		$where = array('superiorId'=>$this->agentId);
		$options	= array(
			'page'		=> $cupage,
			'per_page'	=> $per_page,
			'like'		=> $like,
		...
		$url = base_url('agent/agent?'.$string);
		...
		$this->_template('agent/agentList',$data);  
	} 
	...
	public function show(){
	  ...
	  $this->_template('agent/agentShow',array('item'=>$item)); 
	}  
	...
	public function info(){
		...
		$item = $this->model->getOne($options);
	  $this->_template('agent/agentInfo',array('item'=>$item)); 
	}  
	
	public function save(){

		$data['agentName'] = htmlspecialchars($this->input->get_post('agentname'));
		$data['agentId'] = htmlspecialchars($this->input->get_post('agentid'));
		...
		if($result){
			ajax_return(lang('save_success'),0,'',base_url('agent/agent'));
		}else{
 ...
 */ 
	...
	function ajaxId()
	{
		$id = $this->input->get_post('agentid'); //字段值
		$options = array('select'=>'*');
		if(!empty($id)){
			$options['where'] = array('agentId'=>$id);
		}
		$item = $this->model->getOne($options);
    if(isset($item)&&!empty($item->agentid)){
	...
	function ajaxName()
	{
		$id = $this->input->get_post('agentname'); //字段值
		$options = array('select'=>'*');
		if(!empty($id)){
			$options['where'] = array('agentName'=>$id);
		}
		$item = $this->model->getOne($options);
    if(isset($item)&&!empty($item->agentid)){
	...
	function ajaxShort()
	{
		$short = $this->input->get_post('agentshort'); //字段值
		$id = $this->input->get_post('agentshort'); //字段值
		$options = array('select'=>'*');
		if(!empty($short)){
	...
	function modify_pwd(){
		...
		$id=$this->input->get_post('id');
		$row = $this->model->getOne(array('where'=>array('agentid'=>$id,'superiorId'=>$this->user_info->agentid)));
		if(empty($row))
		{
		  $this->_template('message',array('msg'=>'修改的用户不存在'));
		  return;
		}
		$this->_template('agent/reset',array('id'=>$id));
	}	
	function updatePassword(){
		...
		$options['where']['agentid'] = $this->input->get_post('agentid');
		$options['where']['superiorId'] = $uid;
		...
		//信息返回操作
		if($tof){
		    ajax_return(lang('update_success'),0,'',base_url('agent/agent'));
	    }else{
