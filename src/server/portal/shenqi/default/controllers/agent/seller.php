...
class Seller extends Admin_Controller {
	...
	function index(){
		...
		$url = base_url('agent/seller?'.$string);
		...
		$this->_template('agent/sellerList',$data);  
	} 
	...
	public function show(){
	  ...
	  $this->_template('agent/sellerShow',array('item'=>$item)); 
	}  
	...
	public function info(){
		...
		$item = $this->model->getOne($options);
	  $this->_template('agent/sellerInfo',array('item'=>$item)); 
	}  
	...
	public function save(){
		...
		if($result){
			ajax_return(lang('save_success'),0,'',base_url('agent/seller'));
		}else{
 ...
 */ 
	...
	function modify_pwd(){
		...
		$id=$this->input->get_post('id');
		$row = $this->model->getOne(array('where'=>array('sellerid'=>$id,'agentId'=>$this->user_info->agentid)));
		if(empty($row))
		{
	...
	function updatePassword(){
		...
		//信息返回操作
		if($tof){
		    ajax_return(lang('update_success'),0,'',base_url('agent/seller'));
	    }else{
