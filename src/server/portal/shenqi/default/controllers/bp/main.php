...
class Main extends Admin_Controller { 
    ...
    function __construct()  {  
        parent::__construct(); 
				$this->load->model(array('seller_model'));
	...
	function info(){
		...
		$row =$this->model->getOne(array('sellerid'=>$this->user_info->sellerid));
		$this->_template('bp/sellerInfo',array('item'=>$row));
	}	
	function update()
	{
		...
		$data['sellerid'] = $this->user_info->sellerid;
		$result=$this->model->update($data);
		if($result){
			ajax_return(lang('save_success'),0,'',base_url('bp/main'));
		}else{
