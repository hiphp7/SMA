...
class Agent extends Admin_Controller {
	...
	function index(){
		...
		$url = admin_base_url('agent/agent?'.$string);
		...
		$this->_template('admin/agent/agentList',$data);  
	} 
	...
	public function show(){
	  ...
	  $this->_template('admin/agent/agentShow',array('item'=>$item)); 
	}  
	...
	public function info(){
		...
		$item = $this->model->getOne($options);
	  $this->_template('admin/agent/agentInfo',array('item'=>$item)); 
	}  
	...
	public function save(){
		...
		if($result){
			ajax_return(lang('save_success'),0,'',admin_base_url('agent/agent'));
		}else{
 ...
 */ 
	...
	function modify_pwd(){
		...
		$id=$this->input->get_post('id');
		$row = $this->model->getOne(array('where'=>array('agentid'=>$id,'superiorId'=>$this->agentId)));
		if(empty($row))
		{
	...
	function updatePassword(){
		...
		//信息返回操作
		if($tof){
		    ajax_return(lang('update_success'),0,'',admin_base_url('agent/agent'));
	    }else{
	        ajax_return("密码更新失败");
	    }
	}	    
